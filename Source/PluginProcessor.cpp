...
void SimpleEQAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    juce::dsp::ProcessSpec spec; //Prep filter chain by passing a process spec object through chain

    spec.maximumBlockSize = samplesPerBlock; //mac num of samples to process

    spec.numChannels = 1; // num of channels

    spec.sampleRate = sampleRate; //Sample rate

    leftChain.prepare(spec); //passes to each chain for processing
    rightChain.prepare(spec);

    auto chainSettings = getChainSettings(apvts);
    auto peakCoefficients = juce::dsp::IIR::Coefficients<float>::makePeakFreqFilter(sampleRate, chainSettings.peakFreq, chainSettings.peakQuality, juce::decibelsToGain(chainSettings.peakGainInDecibels));
...
juce::AudioProcessorValueTreeState::ParameterLayout SimpleEQAudioProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>("Peak Quality", "Peak Quality", juce::NormalisableRange<float>(.1f, 10.f, 0.05f, 1.f), 1.f));
